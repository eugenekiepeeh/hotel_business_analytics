...
```{r Lead Time Distribution 2, fig.width = 10, fig.height = 6}
...
```{r}
marketSegment_cancel_df <- final_hotel_bookings |>
  ...
  mutate(
    booking_share = bookings / sum(bookings),
    overall_cancel = cancelBookings/sum(cancelBookings))

distribution_cancel_df <- final_hotel_bookings |>
...
customerType_cancel_df <- final_hotel_bookings |>
  #filter(distribution_channel != "Undefined") |>
